...
class RobotInterface(object):
    def __init__(self, model, data, rfoot_body_name=None, lfoot_body_name=None,
        ...
        # Handle the case where model might not have a body at index 1
        try:
            self.robot_root_name = model.body(1).name
        except IndexError:
            print("Warning: No body at index 1, using body at index 0 as robot root")
            self.robot_root_name = model.body(0).name

        # Check if model loaded correctly - it should have actuators
        if model.nu == 0:
            print("WARNING: Model has no actuators. This might indicate a loading problem.")
            print("Expected actuators for humanoid robot. Trying to check XML content...")
            
            # Check for expected joint names in the model
            expected_joint_names = ["hip_yaw", "hip_roll", "hip_pitch", "knee", "ankle"]
            found_joints = []
            
            for i in range(model.njnt):
                jnt_name = model.joint(i).name
                print(f"Found joint: {jnt_name}")
                
                for expected in expected_joint_names:
                    if expected in jnt_name:
                        found_joints.append(jnt_name)
                        break
            
            if found_joints:
                print(f"Found expected joint patterns in {len(found_joints)} joints")
            else:
                print("No expected joint patterns found - model likely did not load correctly")
                
            # This will be used to override the nu() method if needed
            self._override_nu = 10  # Assuming 10 actuators for H1 robot legs
    ...
    def nu(self):
        # If model loaded properly, return actual value
        if hasattr(self, '_override_nu') and self.model.nu == 0:
            print(f"Using override value for nu(): {self._override_nu}")
            return self._override_nu

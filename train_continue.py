...
config_log = os.path.join(run_log_dir, "training_config_continued.txt")
...
# Apply normalization to eval environment as well
...
if args.continue_from:
    ...
    model = PPO(
        policy="MlpPolicy",
        env=env,
        learning_rate=5e-4,  # Increase learning rate from default
        n_steps=2048,
...
print(f"Training for {args.iterations} iterations ({total_timesteps} total timesteps)")

# Train the agent, reset_num_timesteps will be True by default but can be disabled
model.learn(
    total_timesteps=total_timesteps, 
    callback=[eval_callback, checkpoint_callback, timestep_checkpoint_callback, iteration_metrics_callback],
    reset_num_timesteps=args.reset_num_timesteps
)

# Save the final model and normalization stats
model.save(os.path.join(run_model_dir, "ppo_humanoid_final"))
env.save(os.path.join(run_model_dir, "vec_normalize.pkl"))

print(f"Training run {args.run_name} completed successfully!")
print(f"Final model saved to {os.path.join(run_model_dir, 'ppo_humanoid_final.zip')}") 